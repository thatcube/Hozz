...
public enum SleepIntervals {
    ...
    public static func merge(_ intervals: [DateInterval]) -> [DateInterval] {
        // Delegates rather than repeats. An identical union already lives in
        // HozzCore, where the Mac's own sleep query can reach it — and two
        // copies of one rule is the shape of nearly every wrong number found
        // in this codebase: they agree until they do not, and nothing says
        // which is right.
        TimeUnion.merge(intervals)
    }
