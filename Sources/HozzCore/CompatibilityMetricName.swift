...
/// The stable short names used by Hozz's grouped compatibility payloads.
///
/// This lives in the platform-neutral core because both ends of the wire need
/// the exact same namespace. In particular, a canonical HealthKit tombstone
/// has to name the compatibility metric whose older, timestamp-only identity
/// may still be held by a receiver.
public enum CompatibilityMetricName {
    ...
    public static func typeIdentifier(for name: String) -> String? {
        reversed[name]
    }

    /// Only quantity and category samples ever used timestamp-only identities.
    public static func legacyNamespace(for identifier: String) -> String? {
        guard
            identifier.hasPrefix("HKQuantityTypeIdentifier")
