...
The analysis tools are built to be hard to overstate, because their output goes straight to a language model that will narrate a story around any number. A trend reports "no detectable change" when a flat line fits as well as a sloped one, and refuses below two weeks of days. A correlation puts its interval on an autocorrelation-adjusted sample size, since consecutive days are not independent evidence, and warns when both series are trending. Anomalies use the median and median absolute deviation, and a day with too few records is reported as *the device was not worn* rather than as a low reading. **[docs/mcp.md](docs/mcp.md)** sets out exactly what each tool will and will not claim. It also carries a block you can paste to your assistant to have it configure itself, since every client keeps its MCP settings somewhere different and your assistant already knows where its own live.
