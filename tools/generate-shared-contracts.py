...
RECORD_SCHEMA_PATH = SCHEMA_ROOT / "canonical-record.schema.json"
...
def render_kotlin_contract(mapping: dict, record_schema: dict) -> str:
    records = []
    ...
    root_properties = ",\n".join(
        f"        {quoted(value)}"
        for value in sorted(record_schema["properties"])
    )
    run_properties = ",\n".join(
        "        "
        + quoted(kind)
        ...
        + ", ".join(
            quoted(value)
            for value in ["kind", "schemaVersion", *properties]
        )
        + ")"
        for kind, properties in sorted(
            record_schema["x-hozz-run-kind-properties"].items()
        )
    )
    canonical_common = record_schema["x-hozz-canonical-common-properties"]
    canonical_properties = ",\n".join(
        ...
        + ", ".join(
            quoted(value)
            for value in [*canonical_common, *properties]
        )
        + ")"
        for kind, properties in sorted(
            record_schema["x-hozz-canonical-kind-properties"].items()
        )
    )
    source_record_properties = ", ".join(
        quoted(value)
        for value in sorted(
            record_schema["properties"]["sourceRecord"]["properties"]
        )
    )
    source_properties = ", ".join(
        quoted(value)
        for value in sorted(record_schema["properties"]["source"]["properties"])
    )
    lineage_properties = ", ".join(
        quoted(value)
        for value in sorted(
            record_schema["properties"]["lineage"]["items"]["properties"]
        )
    )
    quantity_properties = ", ".join(
        quoted(value)
        for value in sorted(record_schema["properties"]["quantity"]["properties"])
    )
    value_properties = ", ".join(
        quoted(value)
        for value in sorted(record_schema["$defs"]["valueAndUnit"]["properties"])
    )
...
{canonical_properties}
    ...
    val sourceRecordPropertyNames = setOf({source_record_properties})
    val sourcePropertyNames = setOf({source_properties})
    val lineagePropertyNames = setOf({lineage_properties})
    val quantityPropertyNames = setOf({quantity_properties})
    val valuePropertyNames = setOf({value_properties})
}}
"""
...
def main() -> int:
    ...
    outputs = [
        ...
        (KOTLIN_CONTRACT_PATH, render_kotlin_contract(mapping, record_schema)),
