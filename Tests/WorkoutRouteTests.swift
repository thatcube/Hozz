...
private actor FakeRouteBackend: WorkoutRouteBackend {
    struct Route {
        let id: UUID
        let start: Date
        let end: Date
    ...
    func routeFacts(id: UUID) async throws -> WorkoutRouteFacts? {
        ...
        return WorkoutRouteFacts(startDate: route.start, endDate: route.end)
    }

    nonisolated func locations(
        for routeID: UUID
...
final class WorkoutRouteTests: XCTestCase {
    ...
    func testEveryPointOfARouteIsWrittenOnceAndInOrder() async throws {
        ...
        let reader = WorkoutRouteReader(backend: backend)
    ...
    func testSeveralRoutesAreKeptApart() async throws {
        ...
        let firstPoints = records.filter {
            $0["route"] as? String == first.uuidString.lowercased()
                && $0["kind"] as? String == "workoutRouteLocations"
        }
        let secondPoints = records.filter {
            $0["route"] as? String == second.uuidString.lowercased()
                && $0["kind"] as? String == "workoutRouteLocations"
        }
    ...
    func testResumingMidRouteNeitherRepeatsNorSkipsAPoint() async throws {
        ...
        let first = WorkoutRouteReader(backend: backend)
        var anchor: AnchorToken?
        var records: [[String: Any]] = []
        for _ in 0..<2 {
        ...
        let carried = try WorkoutRouteAnchor.decode(anchor)
        XCTAssertEqual(carried.pendingRoute, id)
        XCTAssertGreaterThan(carried.deliveredLocations, 0)
        XCTAssertLessThan(carried.deliveredLocations, pointCount)

        // A fresh reader over the same cursor is what relaunching looks like.
        let second = WorkoutRouteReader(backend: backend)
        while true {
    ...
    func testAReplayedPageCarriesTheSameIdentifiers() async throws {
        ...
        let straight = WorkoutRouteReader(backend: FakeRouteBackend(routes: routes))
        let whole = try await drainAll(straight)

        let backend = FakeRouteBackend(routes: routes)
        let interrupted = WorkoutRouteReader(backend: backend)
        var anchor: AnchorToken?
        ...
        while true {
            ...
            // re-opened stream.
            reader = WorkoutRouteReader(backend: backend)
        }
    ...
    func testARouteThatDisappearsMidReadIsReportedRatherThanTruncated() async throws {
        ...
        let after = try WorkoutRouteAnchor.decode(batch.proposedAnchor)
        XCTAssertNil(
            after.pendingRoute,
    ...
    func testARouteCursorSurvivesEncoding() throws {
        let id = UUID()
        let anchor = WorkoutRouteAnchor(
            healthKitAnchor: Data([1, 2, 3]),
            pendingRoute: id,
            deliveredLocations: 1_500
        )

        let restored = try WorkoutRouteAnchor.decode(try anchor.token())

        XCTAssertEqual(restored, anchor)
    }

    func testAnEmptyCursorMeansTheStart() throws {
        XCTAssertEqual(try WorkoutRouteAnchor.decode(nil), .start)
    }

    func testAMalformedRouteCursorIsRejectedRatherThanReset() {
        XCTAssertThrowsError(
            try WorkoutRouteAnchor.decode(AnchorToken(data: Data("nonsense".utf8)))
        )
    }
    ...
    func testAnOffsetWithoutARouteIsRejected() throws {
        ...
        XCTAssertThrowsError(
            try WorkoutRouteAnchor.decode(token),
    ...
    func testAPointWithNoFixOmitsTheFieldsItDoesNotHave() {
        ...
        let object = WorkoutRouteEncoding.object(for: location)
    ...
    func testAnExportCarriesEveryPointOfEveryRoute() async throws {
        ...
        let engine = HealthExportEngine(
            store: store,
            source: RouteOnlySource(reader: WorkoutRouteReader(backend: backend)),
            types: [routeType],
    ...
    func testAKilledExportResumesARouteWithoutGapsOrDuplicates() async throws {
        ...
        let carried = try WorkoutRouteAnchor.decode(sealed)
        XCTAssertEqual(
            carried.pendingRoute,
            id,
